...
ofstream fout ("../results/test.txt");
...
{
    string name;
    double rating, weighted_ratings[500], actual_ratings[500], total_weighted_ratings[1000];
    int number_of_games;
};
match_stats* match;
player_stats* player;
        ...
        for(int y=0; y<5; y++){
            for(int p=0; p<total_N_players; p++){
                if(player[p].name == match[match_number].player_name[x][y]){
                    player_index[x][y] = p;
                    if(player[p].number_of_games < N_games_needed) enough_games = false;
                    break;
                }
...
void update_player_ratings(int match_number, double k)
{
    for(int x=0; x<2; x++){
        ...
        for(int y=0; y<5; y++){
            ...
            int game_number = player[p].number_of_games;
...
void output_data()
{
    ofstream output_results ("../results/player_stats.txt");
